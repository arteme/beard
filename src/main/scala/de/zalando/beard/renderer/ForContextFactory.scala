...
object ForContextFactory {
  ...
  def create(forIterationContext: ForIterationContext): Map[String, Any] = {
    ...
    val newContext = forIterationContext.globalContext.updated(
      forIterationContext.templateIteratorIdentifier,
      forIterationContext.collectionContext
    )
    newContext(forIterationContext.templateIteratorIdentifier) match {
      ...
      case other =>
        throw new IllegalAccessException(
          s"We need a map here instead of ${other.getClass} with a value $other " +
          s"for iterator ${forIterationContext.templateIteratorIdentifier}:${forIterationContext.currentIndex}"
        )
    }
  }

  private[renderer] def handleIndexContext(
      forIterationContext: ForIterationContext,
