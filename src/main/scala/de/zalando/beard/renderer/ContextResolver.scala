...
object ContextResolver {
  ...
  def resolveCollection(identifier: CompoundIdentifier, context: Map[String, Any]): Iterable[Any] = {
    ...
    val result = identifier.identifierParts.foldLeft(context.get(identifier.identifierPart)) {
      (ctx: Option[Any], rest: String) =>
        ctx flatMap {
    ...
    result.toSeq flatMap {
      ...
      case other =>
        throw new IllegalStateException(
          s"$identifier does not point to a Iterable or Option but a ${other.getClass} with value: $other"
        )
    }
  }
