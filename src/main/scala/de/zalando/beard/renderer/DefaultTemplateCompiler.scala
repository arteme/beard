...
trait TemplateCompiler {
  def compile(
      ...
      contentForStatements: Map[Identifier, Seq[Statement]] = Map.empty
  ): Try[BeardTemplate]
}
...
class CustomizableTemplateCompiler(
  ...
  def compile(
      ...
      contentForStatementsMap: Map[Identifier, Seq[Statement]] = Map.empty
  ): Try[BeardTemplate] = {
    ...
    beardTemplateTry.flatMap { beardTemplate =>
      val newContentForStatementsMap =
        addContentForStatementsToMap(contentForStatementsMap, beardTemplate.contentForStatements)
  ...
  /**
    ...
    */
  @tailrec
  private[renderer] final def concatTexts(
      statements: Seq[Statement],
      mergedStatements: Seq[Statement] = Seq.empty,
      existingTexts: Seq[HasText] = Seq.empty
  ): Seq[Statement] = statements match {
  ...
  private[renderer] def concatTextsRec(
      statements: Seq[Statement],
      mergedStatements: Seq[Statement] = Seq.empty,
      existingTexts: Seq[HasText] = Seq.empty
  ): Seq[Statement] =
  ...
  private[renderer] def addContentForStatementsToMap(
      contentForStatementsMap: Map[Identifier, Seq[Statement]],
      newContentForStatements: Seq[ContentForStatement]
  ): Map[Identifier, Seq[Statement]] =
  ...
  private def createMergedTemplate(
    ...
    val yieldedBeardTemplate = BeardTemplate(
      newStatements,
...
object DefaultTemplateCompiler
    extends CustomizableTemplateCompiler(
