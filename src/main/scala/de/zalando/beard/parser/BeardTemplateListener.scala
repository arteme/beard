...
class BeardTemplateListener extends BeardParserBaseListener {
  ...
  override def exitText(ctx: TextContext) =
    ctx.result = Text(
      (Option(ctx.TEXT()).map(_.getText).toSeq ++ Option(ctx.CURLY_BRACKET()).map(_.getText).toSeq).head
    )
  ...
  override def exitAttribute(ctx: AttributeContext) = {
    val attributes =
      Option(ctx.attributeWithIdentifier()).map(_.result).toSeq ++ Option(ctx.attributeWithValue()).map(_.result).toSeq
  ...
  override def exitBlockStatement(ctx: BlockStatementContext) =
    ctx.result =
      BlockStatement(ctx.blockInterpolation().identifier().result, ctx.statement().asScala.flatMap(_.result).toList)

  override def exitContentForStatement(ctx: ContentForStatementContext) =
    ctx.result = ContentForStatement(
      ctx.contentForInterpolation().identifier().result,
      ctx.statement().asScala.flatMap(_.result).toList
    )
  ...
  override def exitIfElseStatement(ctx: IfElseStatementContext) = {
    ...
    ctx.result = startingSpaceStatements ++ Seq(
      IfStatement(condition, ifStatements ++ ifSpaceStatements, elseStatements ++ elseSpaceStatements)
    )
  }
  ...
  override def exitUnlessElseStatement(ctx: UnlessElseStatementContext) = {
    ...
    ctx.result = startingSpaceStatements ++ Seq(
      UnlessStatement(condition, unlessStatements ++ unlessSpaceStatements, elseStatements ++ elseSpaceStatements)
    )
  }
  ...
  override def exitBeard(ctx: BeardContext) = {
    ...
    val renderStatements = statements.collect { case renderStatement: RenderStatement =>
      renderStatement
    }
    ctx.result = statements
